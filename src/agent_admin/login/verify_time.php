...
// 計算用
$sql = 'SELECT count(*) FROM agent_password_reset WHERE pass_token = ?';
$stmt = $db->prepare($sql);
$stmt->execute(array($passResetToken));
$result = $stmt->fetch();

// 利用
$sql = 'SELECT * FROM agent_password_reset WHERE pass_token = ?';
$stmt = $db->prepare($sql);
$stmt->execute(array($passResetToken));
$verify = $stmt->fetch();
...
if ($result[0] != 0) {
  ...
  { 
    header('Location: http://localhost/agent_admin/login/reset.php');
    exit;
  } else {
    header('Location: http://localhost/agent_admin/login/expired.php');
  }

  exit;
} else {
  header('Location: http://localhost/agent_admin/warning.php');
}
